#!/usr/bin/env python
"""Phase 6: refine global homographies with matching and rigid energies."""
...
from src.pipeline_io import load_pairs, load_transform_array, save_transform_array, select_connected_subset  # noqa: E402
from src.projective_optimizer import optimize_projectives  # noqa: E402
...
def main() -> int:
    ...
    affine_path = config.cache_dir / f"affine_transforms{suffix}.npy"
    affine_transforms = load_transform_array(affine_path)
    missing = sorted(set(image_ids) - set(affine_transforms))
    if missing:
        raise ValueError(f"Affine cache lacks subset images: {missing}")

    print(f"Reference image: {reference}")
    print(f"Images/valid edges: {len(image_ids)}/{len(observations)}")
    start = time.perf_counter()
    result = optimize_projectives(
        image_ids,
        observations,
        reference,
        affine_transforms,
        sigma_tr=float(optimization.get("sigma_tr", 5000.0)),
        rigid_weight=float(optimization.get("rigid_weight", 800.0)),
        max_nfev=int(optimization.get("max_nfev_projective", 300)),
        verbose=args.verbose if args.verbose is not None else int(optimization.get("verbose", 0)),
    )
    elapsed = time.perf_counter() - start
    transform_path = config.cache_dir / f"projective_transforms{suffix}.npy"
    ...
    geometry = render_average_preview(
        result.transforms, image_paths, image_shapes, config.debug_dir / f"projective_preview{suffix}.jpg",
        scale=preview_scale,
    )
    draw_global_footprints(result.transforms, image_shapes, config.debug_dir / f"projective_footprints{suffix}.png")
    sanity = transformation_sanity_checks(result.transforms, image_shapes)
    global_rmse, metric_rows = projection_rmse(result.transforms, observations)
    metrics_path = config.output_dir / f"projection_rmse{suffix}.csv"
    write_projection_rmse_csv(metrics_path, metric_rows)
    summary = {
        "schema_version": 1,
        "stage": "projective",
        ...
        "initial_selected_match_rmse_px": result.initial_match_rmse_px,
        "final_selected_match_rmse_px": result.final_match_rmse_px,
        "all_inlier_projection_rmse_px": global_rmse,
        "final_match_energy_average": result.final_match_energy_average,
        "final_rigid_energy_average": result.final_rigid_energy_average,
    ...
    summary_path = config.cache_dir / f"projective_summary{suffix}.json"
    write_json_atomic(summary_path, summary)

    print(f"Selected-point match RMSE: {result.initial_match_rmse_px:.3f} -> {result.final_match_rmse_px:.3f} px")
    print(f"All-inlier projection RMSE: {global_rmse:.3f} px")
    print(f"Average energies: match={result.final_match_energy_average:.6f}, rigid={result.final_rigid_energy_average:.6e}")
